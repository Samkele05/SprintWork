...
export async function getLinkedInAccessToken(
  ...
  try {
    const clientId = process.env.LINKEDIN_CLIENT_ID || "YOUR_CLIENT_ID";
    const clientSecret =
      process.env.LINKEDIN_CLIENT_SECRET || "YOUR_CLIENT_SECRET";
...
export async function fetchLinkedInProfile(
  ...
  try {
    ...
    const skillsResponse = await axios.get(
      "https://api.linkedin.com/v2/skills?q=member",
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
    ...
    const profile: LinkedInProfile = {
      ...
      summary: "",
      experience:
        experienceResponse.data.elements?.map((exp: any) => ({
          title: exp.title || "",
      ...
      education:
        educationResponse.data.elements?.map((edu: any) => ({
      ...
      skills:
        skillsResponse.data.elements?.map(
          (skill: any) => skill.name?.localizedName || ""
        ) || [],
      endorsements: {},
 ...
 */
export async function normalizeLinkedInProfile(
  ...
  const educationText = profile.education
    .map(
      edu =>
        `${edu.degree} in ${edu.fieldOfStudy} from ${edu.school} (${edu.startDate}-${edu.endDate})`
    )
    .join("\n");
...
export async function generateProfileSummary(linkedInData: {
  ...
  try {
    ...
    if (typeof content === "string") {
      ...
      return (
        content.map((c: any) => c.text || "").join("") || linkedInData.summary
      );
    }
