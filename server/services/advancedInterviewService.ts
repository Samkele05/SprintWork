...
export const advancedInterviewService = {
  ...
  async generateInterviewQuestions(
    ...
    interviewType: "behavioral" | "technical" | "case_study" | "mixed" = "mixed",
  ...
  calculateReadinessScore(sessionScores: number[]): number {
    ...
    const consistency =
      sessionScores.length > 1
        ? 100 -
          (Math.max(...sessionScores) - Math.min(...sessionScores)) * 2
        : 50;
...
function calculateStructureScore(
  ...
  const transitionCount = (response.match(/then|next|after that|subsequently/gi) || []).length;
  if (transitionCount >= 2) score += 15;
  else if (transitionCount >= 1) score += 8;

  // Check for clear conclusion
  const hasConclusion = /conclusion|summary|learned|takeaway|result|ultimately/i.test(
    response
  );
...
function calculateCommunicationScore(response: string): number {
  ...
  const fillerWords = (response.match(/um|uh|like|you know|basically|actually/gi) || []).length;
  ...
  const professionalTerms = (response.match(/achieved|implemented|developed|managed|led|collaborated/gi) || []).length;
...
function calculateConfidenceScore(response: string): number {
  ...
  const uncertainPhrases = (response.match(/i think|maybe|i guess|sort of|kind of|perhaps/gi) || []).length;
  ...
  const assertivePhrases = (response.match(/i successfully|i ensured|i determined|i decided/gi) || []).length;
  if (assertivePhrases >= 2) score += 15;
  else if (assertivePhrases >= 1) score += 8;

  // Check for ownership language
  const ownershipPhrases = (response.match(/i led|i managed|i owned|i drove/gi) || []).length;
 ...
 */
function extractFeedbackPoints(
  feedback: string
